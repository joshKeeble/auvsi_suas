...
def getLetterMask_scipy(img):
    ...
    letter_mask = cv2.erode(letter_mask, kernel, iterations=2)
...
class KMeans(object):
    ...
    def getLetterMask_cv2(self,crop):
        #
        ...
        K = 4
        while K > 1:
            success, labels, centers = self.calcKMeans(lab_points, K)
            if success:
                break
            K -= 1
...
def main():
    target_directory = '/home/hal/Desktop/Programming/generated_data/shapes/semicircle'
    kmeans = KMeans()
    for image in os.listdir(target_directory):
        image_full_path = os.path.join(target_directory,image)
        if image_full_path.endswith('.jpg'):
            print(image_full_path)

            frame = cv2.imread(image_full_path)
            original = frame.copy()
            #frame = skeletonize(frame)
            #frame = cv2.resize(frame,(00,500))
            #frame = cv2.medianBlur(frame,2)
            #frame = cv2.GaussianBlur(frame,(10,10),0)
            frame = kmeans.getLetterMask_cv2(frame)
            kernel = np.ones((3,3),np.uint8)

            try:

                #for i in range(30):
                    #rotated_frame = rotate(frame,12*i)
                    
                    #rotated_frame = cv2.erode(rotated_frame,kernel,iterations = 1)

                    #frame = cv2.Canny(frame,50,200)
                    #cv2.imshow('frame',255-rotated_frame)
                    #print(rotated_frame)
                    #frame = skeletonize(255-rotated_frame)
                    #frame = skeletonize(np.asarray(frame/255,dtype=np.uint8))  
